## js中为啥要有宏任务和微任务？
* 因为JS中会将所有的任务退入一个队列，队列的执行原则是先进先出，但是会有一些优先级比较高的任务，需要先执行。
## 浏览器的事件循环
        完成一次宏任务后去清空微任务队列  ？？
...
## node中的事件循环和浏览器中的有啥不同         感觉这个说法有问题？？？？？？
...
function cacheDescriptor (target, name, descriptor) {
    ...
    descriptor.value = async function (...argus) {
        ...
        if (!cacheDescriptor.get(itemKey)) {
            const itemValue = Promise.resolve(value.apply(this, argus)).cache((error) => {
...
```
## promise 并发执行3个任务，假设P第一个完成，那就遍历剩下的promise，从中找到一个p，将这个p加入到已完成的p的位置，循环，总是在最先完成的promise的位置添加新的待执行的promise
