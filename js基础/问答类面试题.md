...
- null 是一个特殊关键字 不是标识符 不能被当做变量来使用和赋值;
- undefined 是全局的标识符 可以被赋值和当做变量来使用
...
```js
const oMeta = document.querySelector("meta[name=viewport]");
...
html.style.fontSize = 10 * (html.clientWidth / 320) + "px";
```
...
- 性能： display:none : 修改元素会造成文档回流,读屏器不会读取 display: none 元素内容，性能消耗较大 visibility:hidden: 修改元素只会造成本元素的重绘,性能消耗较少读屏器读取 visibility: hidden 元素内容 opacity: 0 ： 新建图层,性能较高
...
- 会出现在 dom 树当中,但是不会渲染到页面上.其中的元素会被#document-fragment 包裹 使用 content 属性，可以获取 模板所表示的 DOM 树，可以使用 JS 将该元素渲染。
...
- 变量查找
  > 找到变量绑定的地址,根据地址找到内存空间,读取内容
- 变量赋值

  > 会重新开辟新的内存空间,之前旧的内存空间会被 GC 回收掉

- const
  > const 声明的变量与内存之间存在一种绑定的不可变关系,当重新赋值时, js 引擎会尝试去重新分配新的内存 会被拒绝,抛出异常.

#### `git merge git rebase`

- git merge 会保留每个分支的 commit 记录,会形成网状的历史结构

- git rebase 会将分支变更基到要合并的分支 没有个人分支的 coimmit 记录 在代码要合入到主分支之前 使用 rebase 保持和主分支一样的进度

### 数组的 map filter forEach ,方法的区别

#### map

- 原数组的映射,map 的返回值为数组, 将 每个元素传入回调函数后,回调函数执行后的返回结果 作为 map 执行的返回结果.

#### filter find

- filter find 会将数组的每一项元素传入回调函数中,当回调函数执行后返回值为 truth 时,对应的数组元素作为 filter find 的 返回值.
- filter 返回的是数组 find 返回的是 第一个满足回调函数执行返回值为 truth 时,对应的数组元素
- every ,some 也是同理 只是返回的是 boolean

#### forEach

- 返回值为 undefined. 为每一个个数组元素,执行一次回调函数
